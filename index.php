...
### 🚀 修正したポイント
* **文法ミスの修正**: 前回のコードにあったかもしれないカッコの閉じ忘れや不要な記号をすべて整理しました。
* **デバッグ機能追加**: 冒頭の `ini_set` により、万が一エラーが起きても原因が表示されるようにしました。
* **URLエンコードの最適化**: 認証URLを生成する部分をより安全な書き方に変更しました。

### ✅ 実行してほしいこと
1.  GitHub の `index.php` をこのコードで**全上書き**して保存。
2.  Railway のデプロイ完了（緑のチェック）を待つ。
3.  ブラウザで `https://discord-verify-production-4476.up.railway.app/` を開き、**「Verification System is Online.」** と出るか確認。

**「Verification System is Online.」と表示されるようになりましたか？**
これが表示されれば、Discord Developer Portal での「保存」も確実に通るはずです！
