...
if (str_contains($_SERVER['REQUEST_URI'], '/send_button')) {
    ...
    curl_exec($ch);
    exit("Button Sent Successfully.");
}

// --- 機能2: 認証処理 ---
if (!isset($_GET['code'])) {
    exit("Ready. System is running.");
}
...
### 🔍 500エラーを解消するためのポイント
1.  **`str_contains` の使用**: 以前の `$_SERVER['REQUEST_URI'] === '/send_button'` は環境によって判定が厳しすぎることがあるため、より柔軟な記述に変えました。
2.  **エラー表示の追加**: 冒頭の `ini_set` により、もし次もエラーが出た場合、真っ白な画面ではなく「どこが悪いか」が表示されるようになります。
3.  **変数の簡略化**: コードを短くし、サーバーへの負荷とエラーのリスクを減らしました。

### 🚀 やってみること
1.  GitHubの `index.php` を上記の内容で上書きして **Commit**。
2.  Railwayのデプロイが完了したあと、もう一度 **`https://discord-verify-production-4476.up.railway.app/send_button`** を開いてください。

**これで「Button Sent Successfully.」と画面に出ましたか？**
もしまだエラーが出る場合は、画面に出ている「Fatal error: ...」などの文字を教えてください！
